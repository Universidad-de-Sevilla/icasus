...
class valor extends ADOdb_Active_Record 
{
  ...
  public $autorizado;
	...
	{
		$db = $this->DB();
		//comprueba si es responsable del indicador. Por tanto podrá grabar siempre valores de las mediciones 
		//correspondiente a ese indicador indiferente de la entidad a que pertneneca
		$sql2 ="SELECT v.id AS id_valor, i.id AS id_indicador, i.id_responsable AS id_responsable FROM `valores` v
			LEFT JOIN mediciones m ON v.id_medicion = m.id
			LEFT JOIN indicadores i ON m.id_indicador = i.id
			WHERE v.id = $id_valor AND i.id_responsable = $id_usuario_activo";
		$rs2 = $db->execute($sql2);
		...
		if ($rs->_numOfRows == 1 OR $rs2->_numOfRows == 1)
		{
			return  true;
		}
		else
