...
$cod_entidades = array_slice($argv, 1);
// Solicitar confirmación
echo "Migración de los indicadores de las entidades: " . implode(', ', $cod_entidades) . ".\n";
$seguir = readline("¿Continuar con la operación? (s/N): ");
if ($seguir !== "s") {
    exit(0);
}

foreach ($cod_entidades as $cod_entidad) {
    $id_entidad = consultar($ic1, "SELECT id_entidad FROM entidad WHERE codigo = '$cod_entidad';")[0][0] ?? null;
    if (null !== $id_entidad) {
        $ent3 = consultar($ic3, "SELECT * FROM icasus_entidad WHERE codigo = '$cod_entidad';")[0] ?? [];
        ...
        if (null !== $id_entidad3) {
            ...
            foreach (consultar($ic1, "SELECT * FROM indicador WHERE id_entidad = '$id_entidad';") as $ind) {
                ...
                if (null === $ind3) {
                    $sql = "INSERT INTO icasus_indicador (id_entidad, id_proceso, id_responsable, id_responsable_medicion, "
                        ...
                        . "'" . utf8_encode($ind['nombre']) . "', "
                        . "'" . utf8_encode($ind['descripcion']) . "', "
                        . "'" . utf8_encode($ind['formulacion']) . "', "
                        ...
                        . "'" . utf8_encode($ind['fuente']) . "', 0, '" . utf8_encode($ind['fuente']) . "', "
            ...
            echo "*** La entidad con código '$cod_entidad' no existe en Icasus3.\n";
        }
    } else {
        echo "*** La entidad con código '$cod_entidad' no existe en Icasus1.\n";
    }
}
