...
if ($modulo == 'subir')
{
  ...
  {       
		if (!mkdir($dir, 0755))
		{
			$error = "Error no se ha podido crear el directorio tipo objeto";
	...
	if (!file_exists($dir.$subdir))
	{
		...
		{
			$error = "Error no se ha podido crear el subdirectorio objeto";
			header("location:index.php?page=archivo_gestionar&id_proceso=$fichero->id_objeto&error=$error");
		}
	}
	//grabamos el archivo
	if($fichero->save())
  {
		...
		{
			$aviso = "Se ha subido correctamente el archivo";
			header("location:index.php?page=archivo_gestionar&id_proceso=$fichero->id_objeto&aviso=$aviso");
		}
	}
	else
	{
			$error = "Error no se han podido grabar los datos del archvio";
			header("location:index.php?page=archivo_gestionar&id_proceso=$fichero->id_objeto&error=$error");
	}
}
