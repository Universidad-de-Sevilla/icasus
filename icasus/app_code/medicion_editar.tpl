      ...
      {if isset($valores_referencia_medicion)}
        {foreach $valores_referencia_medicion as $valor_referencia_medicion}
          <fieldset class="label_side">
            ...
            <a href="javascript:void(0)" onclick="referencia_editar('{$valor_referencia_medicion->id}')">{if $valor_referencia_medicion->valor == NULL}---{else}{$valor_referencia_medicion->valor|string_format:"%.2f"}{/if}</a></span></div>
          </fieldset>
        {/foreach}
      {else}
        <fieldset class="label_side">
    ...
    {else}
      ...
      {if isset($valores_referencia_medicion)}
        {foreach $valores_referencia_medicion as $valor_referencia_medicion}
          <fieldset class="label_side">
            <label>{$valor_referencia_medicion->valor_referencia->etiqueta}</label>
            <div>{$valor_referencia_medicion->valor|string_format:"%.2f"}</div>
          </fieldset>
        {/foreach}
      {else}
        <fieldset class="label_side">
