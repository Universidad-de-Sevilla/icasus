...
{foreach from=$grupos_operario item=grupo}
   <tr>
    <td>{$grupo.codigo}</td>
    <td>{$grupo.nombre}</td>
    <td>{if $grupo.rol =='secretario'}
        <a href='index.php?page=grupo/grupo_datos&id_entidad={$grupo.id_entidad}' title="Ver y editar detalles del equipo">Editar</a> 
	{else}
        <a href='index.php?page=grupo/grupo_ver&id_entidad={$grupo.id_entidad}' title="Ver equipo">Ver</a>
	{/if}
   </td>		
   <td><a href='index.php?page=indicador_listar&id_entidad={$grupo.id_entidad}'>Indicadores</a> </td>
   <td><a href="index.php?page=plan_estrategico/plan_listar&id_entidad={$grupo.id_entidad}&entidad={$grupo.nombre|htmlentities}">Planificaci&oacute;n</a></td> 
   <td><a href='index.php?page=grupo/acta_listar&id_entidad={$grupo.id_entidad}' title="Ver las actas de este equipo">Actas</a></td>
   </tr>
{/foreach}
</tbody></table>
{/if}
