    ...
    <div class="col-lg-12">
        <form action='index.php?page=carta_grabar' method='post' id='formcarta' name='formcarta' 
            ...
            <div class="form-group has-feedback">
                <label for="fecha" class="col-sm-2 control-label"><i title="{$smarty.const.MSG_CAMPO_REQ}" class="fa fa-asterisk fa-fw"></i> {$smarty.const.FIELD_FECHA}</label>
                <div class="col-sm-8">
                    <div class="input-group date" data-provide="datepicker" data-date-language="es" data-date-autoclose="true" data-date-today-highlight="true">
                        ...
                        <input type='text' value="{$carta->fecha|date_format:'%d/%m/%Y'}" class="form-control" name='fecha' id='fecha' placeholder="{$smarty.const.FIELD_FECHA}" data-validar_fecha="validar_fecha" required/>
                    </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    <textarea  class="form-control" name="mision" id="mision" placeholder="{$smarty.const.FIELD_MISION}">{$carta->mision}</textarea>
                </div>
            </div>
            <div class="form-group">
                <label for="vision" class="col-sm-2 control-label">{$smarty.const.FIELD_VISION}</label>
                <div class="col-sm-8">
                    <textarea  class="form-control" name="vision" id="vision" placeholder="{$smarty.const.FIELD_VISION}">{$carta->vision}</textarea>
                </div>
            </div>
            <div class="form-group">
                <label for="valores" class="col-sm-2 control-label">{$smarty.const.FIELD_VALORES}</label>
                <div class="col-sm-8">
                    <textarea  class="form-control" name="valores" id="valores" placeholder="{$smarty.const.FIELD_VALORES}">{$carta->valores}</textarea>
                </div>
            </div>
            <div class="form-group">
                <div class="col-sm-offset-2 col-sm-8">
                    <a class="btn btn-danger" title="{$smarty.const.TXT_CANCEL}" href ='index.php?page=carta_mostrar&id_carta={$carta->id}&id_entidad={$entidad->id}'>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h3 class="modal-title" id="myModalLabel"><i class="fa fa-map-o fa-fw"></i><sub class="fa fa-pencil fa-fw"></sub> {$smarty.const.TXT_CARTA_EDIT}: {$carta->fecha|date_format:'%d/%m/%Y'}</h3>
                        </div>
