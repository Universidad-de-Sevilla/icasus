...
<div class="row">
    <div class="col-lg-12">
        <h3 title="{$_nombre_pagina}" class="page-header">
            <i class="fa fa-gavel fa-fw"></i><sub class="fa fa-pencil fa-fw"></sub> {$_nombre_pagina}
        </h3>
    </div>
    <!-- /.col-lg-12 -->
</div>
...
<div class="row">
    <div class="col-lg-12">
        <form action='index.php?page=derecho_grabar' method='post' id='formderecho' name='formderecho' 
              data-toggle="validator" class="form-horizontal">
            <input type="hidden" name="id_carta" value="{$carta->id}">
            <input type="hidden" name="id_derecho" value="{$derecho->id}">
            <div class="form-group has-feedback">
                ...
                <div class="col-sm-8">
                    <input type='text' class="form-control" value="{$derecho->nombre}" name='nombre' id='nombre' placeholder="{$smarty.const.FIELD_NOMBRE}" required/>
                    <span class="glyphicon form-control-feedback" aria-hidden="true"></span>
                    <div class="help-block with-errors"></div>
                </div>
            </div>
            <div class="form-group">
                <label for="categoria" class="col-sm-2 control-label">{$smarty.const.FIELD_CAT}</label>
                <div class="col-sm-8">
                    <input type='text' class="form-control" value="{$derecho->categoria}" name='categoria' id='categoria' placeholder="{$smarty.const.FIELD_CAT}"/>
                </div>
            </div>
            <div class="form-group">
                <label for="descripcion" class="col-sm-2 control-label">{$smarty.const.FIELD_DESC}</label>
                <div class="col-sm-8">
                    <textarea  class="form-control" id="descripcion" name="descripcion" placeholder="{$smarty.const.FIELD_DESC}">{$derecho->descripcion}</textarea>
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h3 class="modal-title" id="myModalLabel"><i class="fa fa-gavel fa-fw"></i><sub class="fa fa-pencil fa-fw"></sub> {$smarty.const.TXT_DERECHO_EDIT}: {$derecho->nombre}</h3>
                        </div>
                        <div class="modal-body">
                            <p>{$smarty.const.MSG_DERECHO_CONFIRM_EDITAR}</p>
                        </div>
