        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title" id="myModalLabel"><i
                            class="fa fa-trash fa-fw"></i> {$smarty.const.TXT_INDIC_BORRAR}: {$indicador->nombre}</h3>
            </div>
            ...
            <div class="modal-footer">
                ...
                <a title="{$smarty.const.TXT_SI}" class="btn btn-success" name="borrar" id="borrar"
                   href='index.php?page=indicador_borrar&id_indicador={$indicador->id}&id_entidad={$indicador->id_entidad}'><i
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title" id="myModalLabel"><i
                            class="fa fa-archive fa-fw"></i> {$smarty.const.TXT_INDIC_ARCHIVAR}: {$indicador->nombre}
                </h3>
            </div>
            <div class="modal-body">
            ...
            <div class="modal-footer">
                <button type="button" title="{$smarty.const.TXT_NO}" class="btn btn-danger" data-dismiss="modal"><i
                            class="fa fa-times fa-fw"></i> {$smarty.const.TXT_NO}</button>
                <a title="{$smarty.const.TXT_SI}" class="btn btn-success" name="borrar" id="borrar"
                   href='index.php?page=indicador_archivar&id_indicador={$indicador->id}&id_entidad={$indicador->id_entidad}&modulo=archivar'><i
                            class="fa fa-check fa-fw"></i> {$smarty.const.TXT_SI}</a>
            </div>
        </div>
    </div>
                        ...
                        <div class="btn-toolbar" role="toolbar" aria-label="">
                            <div class="btn-group" role="group" aria-label="">
                                <a title="{$smarty.const.TXT_PRIMER} {$smarty.const.FIELD_INDIC}"
                                   ...
                                   href='index.php?page=indicador_mostrar&id_entidad={$entidad->id}&id_indicador={$indicadores[0]->id}'>
                                    <i class="fa fa-step-backward fa-fw"></i>
                                </a>
                                {if count($indicadores) > 2}
                                    <a title="{$smarty.const.TXT_ANT} {$smarty.const.FIELD_INDIC}"
                                       class="btn btn-danger btn-xs {if $indice == 0}disabled{/if}"
                                       href='index.php?page=indicador_mostrar&id_entidad={$entidad->id}&id_indicador={$indicadores[$indice-1]->id}'>
                                ...
                                <a title="{$smarty.const.TXT_ULTIMO} {$smarty.const.FIELD_INDIC}"
                                   class="btn btn-danger btn-xs {if $indice == (count($indicadores)-1)}disabled{/if}"
                                   href='index.php?page=indicador_mostrar&id_entidad={$entidad->id}&id_indicador={$indicadores[(count($indicadores)-1)]->id}'>
    ...
    <div class="col-lg-12">
        <ul class="nav nav-tabs">
            <li role="presentation" class="active">
                <a title="{$smarty.const.TXT_FICHA}" href="#"><i
...
{if $_control || $responsable}
    <div id="botones" class="hidden">
        <a title="{$smarty.const.TXT_INDIC_EDIT}" class="btn btn-danger {if $indicador->archivado}disabled{/if}"
        ...
        {/if}
        <a title="{$smarty.const.TXT_INDIC_BORRAR}" class="btn btn-danger {if $indicador->archivado}disabled{/if}"
                                    ...
                                    {foreach $objops as $objop}
                                        <li>
                                            <a title="{$smarty.const.TXT_FICHA}"
                                               href="index.php?page=objop_mostrar&id_objop={$objop->id}&id_entidad={$indicador->id_entidad}">{$objop->nombre}</a>
                                        </li>
                                    {/foreach}
                                </ul>
                            {else}
                        ...
                        <td>
                            {if $indicador->id_proceso}
                                <a title="{$smarty.const.TXT_PROC_VER}"
                                   href="index.php?page=proceso_mostrar&id_proceso={$indicador->proceso->id}&id_entidad={$indicador->id_entidad}">{$indicador->proceso->codigo}
                        ...
                        {if ($indicador->criterios_efqm|@count) > 0}
                            <ul class="list-unstyled">
                                {foreach $indicador->criterios_efqm as $indicador_criterio_efqm}
                                    <li>
                                        {$indicador_criterio_efqm->criterio_efqm->codigo}
                                        - {$indicador_criterio_efqm->criterio_efqm->nombre}
                                    </li>
                                {/foreach}
                            </ul>
                        {else}
                        ...
                        {if $indicador_subunidades}
                            <ul class="list-unstyled">
                                {foreach $indicador_subunidades as $indicador_subunidad}
                                    <li>
                                        <a title="{$indicador_subunidad->entidad->etiqueta}"
                                           href="index.php?page=entidad_mostrar&id_entidad={$indicador_subunidad->entidad->id}">{$indicador_subunidad->entidad->etiqueta}</a>
                                    </li>
                                {/foreach}
                            </ul>
                        {else}
