        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title" id="myModalLabel"><i
                            class="fa fa-trash fa-fw"></i> {$smarty.const.TXT_MED_BORRAR}: <span
                            id="etiqueta_medicion"></span></h3>
            </div>
...
<div class="modal fade" id="dialogo_confirmar_borrado_mediciones" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title" id="myModalLabel"><i
                            class="fa fa-trash fa-fw"></i> {$smarty.const.TXT_MEDS_BORRAR}</h3>
            </div>
            <div class="modal-body">
            ...
            <div class="modal-footer">
                <button type="button" title="{$smarty.const.TXT_NO}" class="btn btn-danger" data-dismiss="modal"><i
                            class="fa fa-times fa-fw"></i> {$smarty.const.TXT_NO}</button>
                <a title="{$smarty.const.TXT_SI}" class="btn btn-success" onClick="$('#form_mediciones').submit();"><i
                            class="fa fa-check fa-fw"></i> {$smarty.const.TXT_SI}</a>
            </div>
        </div>
    </div>
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title" id="myModalLabel"><i class="fa fa-hourglass fa-fw"></i><sub
                            class="fa fa-plus fa-fw"></sub> {$smarty.const.TXT_MED_CREAR}: {$indicador->nombre}</h3>
            </div>
            <form class="form-horizontal" id="form_generar" name="form_generar" data-toggle="validator" method="post"
                  action="index.php?page=medicion_generar&id_entidad={$indicador->id_entidad}">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group has-feedback">
                        <label for="anyo" class="col-sm-2 control-label"><i title="{$smarty.const.MSG_CAMPO_REQ}"
                                                                            class="fa fa-asterisk fa-fw"></i> {$smarty.const.FIELD_ANYO}
                        </label>
                        <div class="col-sm-4">
                            <input type="number" value="{$smarty.now|date_format:'%Y'}" name="anyo" id='anyo'
                                   class="form-control" placeholder="{$smarty.const.FIELD_ANYO}"
                                   min='{$indicador->historicos}' max="{$smarty.now|date_format:'%Y'}" required/>
                ...
                <div class="modal-footer">
                    <button type="button" title="{$smarty.const.TXT_NO}" class="btn btn-danger" data-dismiss="modal"><i
                                class="fa fa-times fa-fw"></i> {$smarty.const.TXT_NO}</button>
                    <button id="btn_confirm_generar" type="submit" title="{$smarty.const.TXT_SI}"
                            data-texto_generando="{$smarty.const.MSG_MED_GENERANDO}" class="btn btn-success"><i
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title" id="myModalLabel"><i class="fa fa-hourglass fa-fw"></i><sub
                            class="fa fa-arrow-circle-up fa-fw"></sub> {$smarty.const.TXT_MED_CARGAR}
                    : {$indicador->nombre}</h3>
            </div>
            <form class="form-horizontal" id="form_cargar" name="form_cargar" data-toggle="validator" method="post"
                  action="index.php?page=medicion_listar&id_indicador={$indicador->id}&id_entidad={$indicador->id_entidad}">
                <div class="modal-body">
                    <p>{$smarty.const.MSG_MED_CARGAR}</p>
                    <div class="form-group has-feedback">
                        <label for="registros" class="col-sm-3 control-label"><i title="{$smarty.const.MSG_CAMPO_REQ}"
                                                                                 class="fa fa-asterisk fa-fw"></i> {$smarty.const.FIELD_REGISTROS}
                        </label>
                        <div class="col-sm-4">
                            <input type="number" value="{$total_registros}" name="registros" id='registros'
                                   class="form-control" placeholder="{$smarty.const.FIELD_REGISTROS}" min='1'
                                   max="{$total_registros}" required/>
                            <span class="glyphicon form-control-feedback" aria-hidden="true"></span>
                            <div class="help-block with-errors"></div>
                        </div>
                    </div>
                ...
                <div class="modal-footer">
                    <button type="button" title="{$smarty.const.TXT_NO}" class="btn btn-danger" data-dismiss="modal"><i
                                class="fa fa-times fa-fw"></i> {$smarty.const.TXT_NO}</button>
                    <button id="btn_confirm_cargar" type="submit" title="{$smarty.const.TXT_SI}"
                            data-texto_cargando="{$smarty.const.MSG_MED_CARGANDO}" class="btn btn-success"><i
                                class="fa fa-check fa-fw"></i> {$smarty.const.TXT_SI}</button>
                </div>
            </form>
        </div>
            ...
            <div class="row">
                <div class="col-md-10">
                    <h4><i class="fa fa-dashboard fa-fw"></i>{if $indicador->archivado}<sub
                        ...
                        <sub
                                class="fa fa-gear fa-fw"></sub>
                    {elseif $indicador->control}
                        <sub
                                class="fa fa-sliders fa-fw"></sub>
                    {else}
                        <sub
                                class="fa fa-database fa-fw"></sub>
                    {/if}{/if} {$_nombre_pagina}
                    </h4>
                </div>
                <!-- /.col-md-10 -->
                <!-- Navegación -->
                        ...
                        <div class="btn-toolbar" role="toolbar" aria-label="">
                            <div class="btn-group" role="group" aria-label="">
                                <a title="{$smarty.const.TXT_PRIMER} {$smarty.const.FIELD_INDIC}"
                                   ...
                                   href='index.php?page=medicion_listar&id_entidad={$entidad->id}&id_indicador={$indicadores[0]->id}'>
                                    <i class="fa fa-step-backward fa-fw"></i>
                                </a>
                                <a title="{$smarty.const.TXT_ANT} {$smarty.const.FIELD_INDIC}"
                                   class="btn btn-danger btn-xs {if $indice == 0}disabled" href="#"{else}"
                                href='index.php?page=medicion_listar&id_entidad={$entidad->id}&id_indicador={$indicadores[$indice-1]->id}'{/if}>
                                ...
                                <a title="{$smarty.const.TXT_ULTIMO} {$smarty.const.FIELD_INDIC}"
                                   ...
                                   href='index.php?page=medicion_listar&id_entidad={$entidad->id}&id_indicador={$indicadores[(count($indicadores)-1)]->id}'>
    ...
    <div class="col-lg-12">
        <ul class="nav nav-tabs">
            ...
            <li role="presentation" class="active">
                <a title="{$smarty.const.FIELD_MEDICIONES}" href="#"><i
...
<div class="row">
    {if ($_control || $responsable) && !$indicador->calculo}
        {if $mediciones}
            <div class="col-lg-6">
                <a title="{$smarty.const.TXT_MED_CREAR}"
                   class="btn btn-danger btn-block {if $indicador->archivado}disabled{/if}" href='javascript:void(0)'
                   data-toggle="modal" data-target="#dialogo_confirmar_generar">
                    <i class="fa fa-hourglass fa-fw"></i><sub
                            class="fa fa-plus fa-fw"></sub> {$smarty.const.TXT_MED_CREAR}
                </a>
            </div>
            <!-- /.col-lg-6 -->
            <div class="col-lg-6">
...
<div class="row">
    <div class="col-lg-12">
        <div class="panel panel-red">
            <div class="panel-heading">
                <span class="panel-title"><i
                            class="fa fa-hourglass fa-fw"></i> {$smarty.const.TXT_MED_TABLA} {if $mediciones}({$smarty.const.TXT_MOSTRANDO}: {if $limite>=$total_registros}{$smarty.const.TXT_REGS_TODOS}{else}{$limite} {$smarty.const.TXT_DE} {$total_registros} {$smarty.const.FIELD_REGISTROS}{/if}){/if}</span>
            ...
            <div class="panel-body">
                ...
                {if $mediciones && ($_control || $responsable)}
                    <div id="botones" class="btn-toolbar hidden" role="toolbar" aria-label="">
                        <div class="btn-group" role="group" aria-label="">
                            <a title="{$smarty.const.TXT_MEDS_BORRAR}"
                               class="btn btn-default btn-danger {if $indicador->archivado}disabled{/if}"
                               href='javascript:void(0)' data-toggle="modal"
                               data-target="#dialogo_confirmar_borrado_mediciones">
                ...
                {if $mediciones}
                    <form id="form_mediciones" action='index.php?page=medicion_borrar&id_entidad={$entidad->id}'
                          method='post'>
                        <input type="hidden" name="id_indicador" value="{$indicador->id}"/>
                        <div class="table-responsive">
                            <table id="tabla_mediciones" class="table datatable table-striped table-hover">
                                ...
                                {foreach $mediciones as $medicion}
                                    <tr {if isset($status[$medicion->id])}class="{$status[$medicion->id]}"{/if}>
                                        ...
                                        <td>
                                            <a title="{$smarty.const.TXT_MED_VER}"
                                               href="index.php?page=medicion&id_entidad={$entidad->id}&id_medicion={$medicion->id}">{$medicion->etiqueta}</a>
                                        </td>
                                            ...
                                            {if ($totales[$medicion->id])!== NULL}
                                                {if isset($status[$medicion->id])}
                                                    {if $status[$medicion->id] == 'danger'}
                                                        <span title="{if $indicador->inverso}{$smarty.const.TXT_DESCENDENTE}{else}{$smarty.const.TXT_ASCENDENTE}{/if}: {$smarty.const.TXT_VAL_MEJORABLE} ({$smarty.const.FIELD_LIMITE}: {$medicion_lim[$medicion->id]}, {$smarty.const.FIELD_META}: {$medicion_obj[$medicion->id]})"
                                                              class="label label-{$status[$medicion->id]}">{$totales[$medicion->id]|round:"2"}</span>
                                                    {elseif $status[$medicion->id] == 'success'}
                                                        <span title="{if $indicador->inverso}{$smarty.const.TXT_DESCENDENTE}{else}{$smarty.const.TXT_ASCENDENTE}{/if}: {$smarty.const.TXT_VAL_LOGRADO} ({$smarty.const.FIELD_LIMITE}: {$medicion_lim[$medicion->id]}, {$smarty.const.FIELD_META}: {$medicion_obj[$medicion->id]})"
                                                              class="label label-{$status[$medicion->id]}">{$totales[$medicion->id]|round:"2"}</span>
                                                    {else}
                                                        <span title="{if $indicador->inverso}{$smarty.const.TXT_DESCENDENTE}{else}{$smarty.const.TXT_ASCENDENTE}{/if}: {$smarty.const.TXT_VAL_ACEPTABLE} ({$smarty.const.FIELD_LIMITE}: {$medicion_lim[$medicion->id]}, {$smarty.const.FIELD_META}: {$medicion_obj[$medicion->id]})"
                                                              class="label label-{$status[$medicion->id]}">{$totales[$medicion->id]|round:"2"}</span>
                                                    {/if}
                                                {else}
                                        ...
                                        <td>
                                            <a class="btn btn-default btn-circle btn-xs"
                                               title="{$smarty.const.TXT_MED_VER}"
                                               href="index.php?page=medicion&id_entidad={$entidad->id}&id_medicion={$medicion->id}"><i
                                                        class="fa fa-pencil fa-fw"></i></a>
                                            {if $_control || $responsable}
                                                <a class="btn btn-default btn-circle btn-xs {if $indicador->archivado}disabled{/if}"
                                                   title="{$smarty.const.TXT_MED_BORRAR}" href='javascript:void(0)'
                                                   data-toggle="modal" data-target="#dialogo_confirmar_borrado"
                                                   data-etiqueta_medicion="{$medicion->etiqueta}"
                                                   data-id_medicion="{$medicion->id}" data-id_entidad="{$entidad->id}">
                                                    <i class="fa fa-trash fa-fw"></i>
                                                </a>
                                            {/if}
                                        </td>
...
{if $indicador->id_tipo_agregacion_temporal != 0}
    <div class="row">
        <div class="col-lg-12">
            <div class="panel panel-red">
                <div class="panel-heading">
                    <span class="panel-title"><i class="fa fa-tags fa-fw"></i> {$smarty.const.TXT_VAL_TABLA}
                        : {$smarty.const.FIELD_UNID}
                        /{$smarty.const.FIELD_ANYO} {if $mediciones}({$smarty.const.TXT_DESDE} {$anyo_inicio}){/if}</span>
                    <i class="fa fa-chevron-up pull-right clickable"></i>
                </div>
                <!-- /.panel-heading -->
                <div class="panel-body">
                        ...
                        <table class="table table-striped table-hover tabla_valores">
                            ...
                            {if $indicador->id_tipo_agregacion!=0}
                                ...
                                {foreach from=$subunidades_mediciones item=subunidades}
                                    {if $subunidades->id == $entidad->id}
                                        <tr>
                                            <th>{$smarty.const.FIELD_TOTAL}: {$subunidades->etiqueta}
                                                ({$agregacion_temporal})
                                            </th>
                                            {for $i=($smarty.now|date_format:"%Y") to $anyo_inicio step -1}
                                                        ...
                                                        {if isset($status_anuales[$subunidades->id][$i])}
                                                            {if $status_anuales[$subunidades->id][$i] == 'danger'}
                                                                <span title="{if $indicador->inverso}{$smarty.const.TXT_DESCENDENTE}{else}{$smarty.const.TXT_ASCENDENTE}{/if}:
                                                                        ...
                                                                        {$ref_anuales_obj[$i]})"
                                                                      class="label label-{$status_anuales[$subunidades->id][$i]}">{$totales_anuales[$subunidades->id][$i]|round:"2"}</span>
                                                            {elseif $status_anuales[$subunidades->id][$i] == 'success'}
                                                                <span title="{if $indicador->inverso}{$smarty.const.TXT_DESCENDENTE}{else}{$smarty.const.TXT_ASCENDENTE}{/if}: {$smarty.const.TXT_VAL_LOGRADO} ({$smarty.const.FIELD_LIMITE}: {$ref_anuales_lim[$i]}, {$smarty.const.FIELD_META}: {$ref_anuales_obj[$i]})"
                                                                      class="label label-{$status_anuales[$subunidades->id][$i]}">{$totales_anuales[$subunidades->id][$i]|round:"2"}</span>
                                                            {else}
                                                                <span title="{if $indicador->inverso}{$smarty.const.TXT_DESCENDENTE}{else}{$smarty.const.TXT_ASCENDENTE}{/if}: {$smarty.const.TXT_VAL_ACEPTABLE} ({$smarty.const.FIELD_LIMITE}: {$ref_anuales_lim[$i]}, {$smarty.const.FIELD_META}: {$ref_anuales_obj[$i]})"
                                                                      class="label label-{$status_anuales[$subunidades->id][$i]}">{$totales_anuales[$subunidades->id][$i]|round:"2"}</span>
                                                            {/if}
                                                        {else}
                                            ...
                                            {for $i=($smarty.now|date_format:"%Y") to $anyo_inicio step -1}
                                                <td {if isset($status_anuales[$subunidades->id][$i])}class="{$status_anuales[$subunidades->id][$i]}"{/if}>
                                                    {if ($totales_anuales[$subunidades->id][$i])!== NULL}
                                                        {if isset($status_anuales[$subunidades->id][$i])}
                                                            {if $status_anuales[$subunidades->id][$i] == 'danger'}
                                                                <span title="{if $indicador->inverso}{$smarty.const.TXT_DESCENDENTE}{else}{$smarty.const.TXT_ASCENDENTE}{/if}: {$smarty.const.TXT_VAL_MEJORABLE} ({$smarty.const.FIELD_LIMITE}: {$ref_anuales_lim[$i]}, {$smarty.const.FIELD_META}: {$ref_anuales_obj[$i]})"
                                                                      class="label label-{$status_anuales[$subunidades->id][$i]}">{$totales_anuales[$subunidades->id][$i]|round:"2"}</span>
                                                            {elseif $status_anuales[$subunidades->id][$i] == 'success'}
                                                                <span title="{if $indicador->inverso}{$smarty.const.TXT_DESCENDENTE}{else}{$smarty.const.TXT_ASCENDENTE}{/if}: {$smarty.const.TXT_VAL_LOGRADO} ({$smarty.const.FIELD_LIMITE}: {$ref_anuales_lim[$i]}, {$smarty.const.FIELD_META}: {$ref_anuales_obj[$i]})"
                                                                      class="label label-{$status_anuales[$subunidades->id][$i]}">{$totales_anuales[$subunidades->id][$i]|round:"2"}</span>
        ...
        <div class="col-lg-12">
            <div class="panel panel-red">
                <div class="panel-heading">
                    <span class="panel-title"><i class="fa fa-tags fa-fw"></i> {$smarty.const.TXT_VAL_TABLA}
                        : {$smarty.const.FIELD_UNID}
                        /{$smarty.const.FIELD_MED} {if $mediciones}({$smarty.const.TXT_MOSTRANDO}: {if $limite>=$total_registros}{$smarty.const.TXT_REGS_TODOS}{else}{$limite} {$smarty.const.TXT_DE} {$total_registros} {$smarty.const.FIELD_REGISTROS}{/if}){/if}</span>
                    <i class="fa fa-chevron-up pull-right clickable"></i>
                </div>
                <!-- /.panel-heading -->
                <div class="panel-body">
