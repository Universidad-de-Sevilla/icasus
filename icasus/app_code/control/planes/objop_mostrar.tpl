    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title" id="myModalLabel"><i class="fa fa-bullseye fa-fw"></i><sub
                            class="fa fa-tasks fa-fw"></sub> {$smarty.const.FIELD_OBJ_OP}
                    : {$objop->objest->linea->indice}.{$objop->objest->indice}.{$objop->indice}. {$objop->nombre}</h3>
            </div>
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title" id="myModalLabel"><i
                            class="fa fa-trash fa-fw"></i> {$smarty.const.TXT_OBJOP_BORRAR}
                    : {$objop->objest->linea->indice}.{$objop->objest->indice}.{$objop->indice}. {$objop->nombre}</h3>
            </div>
            <div class="modal-body">
            ...
            <div class="modal-footer">
                ...
                <a title="{$smarty.const.TXT_SI}" class="btn btn-success" name="borrar" id="borrar"
                   href="index.php?page=objop_borrar&id_objop={$objop->id}&id_entidad={$entidad->id}"><i
                    ...
                    <div class="col-md-2">
                        ...
                        <div class="btn-toolbar" role="toolbar" aria-label="">
                            <div class="btn-group" role="group" aria-label="">
                                <a title="{$smarty.const.TXT_PRIMER} {$smarty.const.FIELD_OBJ_OP}"
                                   class="btn btn-danger btn-xs {if $indice == 0}disabled{/if}"
                                   href='index.php?page=objop_mostrar&id_entidad={$entidad->id}&id_objop={$objops[0]->id}'>
                                    <i class="fa fa-step-backward fa-fw"></i>
                                </a>
                                <a title="{$smarty.const.TXT_ANT} {$smarty.const.FIELD_OBJ_OP}"
                                   class="btn btn-danger btn-xs {if $indice == 0}disabled" href="#"{else}"
                                   href='index.php?page=objop_mostrar&id_entidad={$entidad->id}&id_objop={$objops[$indice-1]->id}'{/if}>
                                    <i class="fa fa-play fa-rotate-180 fa-fw"></i>
                                </a>
                                <a title="{$smarty.const.TXT_SIG} {$smarty.const.FIELD_OBJ_OP}"
                                   class="btn btn-danger btn-xs {if $indice == (count($objops)-1)}disabled" href="#"{else}"
                                   href='index.php?page=objop_mostrar&id_entidad={$entidad->id}&id_objop={$objops[$indice+1]->id}'{/if}>
                                    <i class="fa fa-play fa-fw"></i>
                                </a>
                                <a title="{$smarty.const.TXT_ULTIMO} {$smarty.const.FIELD_OBJ_OP}"
                                   class="btn btn-danger btn-xs {if $indice == (count($objops)-1)}disabled{/if}"
                                   href='index.php?page=objop_mostrar&id_entidad={$entidad->id}&id_objop={$objops[(count($objops)-1)]->id}'>
...
<div class="row">
    <div class="col-lg-12">
        <ol class="breadcrumb">
            ...
            <li><a title="{$smarty.const.FIELD_OBJ_EST} {$linea->indice}.{$objest->indice}. {$objest->nombre}"
                   href='index.php?page=objest_mostrar&id_objest={$objest->id}&id_entidad={$entidad->id}'>{$smarty.const.FIELD_OBJ_EST} {$linea->indice}
                    .{$objest->indice}. {$objest->nombre|truncate:30}</a></li>
            <li><a title="{$smarty.const.FIELD_OBJS_OP}"
                   href='index.php?page=objest_mostrar&id_objest={$objest->id}&id_entidad={$entidad->id}#objest_objops'>{$smarty.const.FIELD_OBJS_OP}</a>
            </li>
    ...
    <div class="col-lg-12">
        ...
        <ul class="nav nav-tabs" role="tablist">
            <li role="presentation" class="active">
                <a href="#objop_ficha" title="{$smarty.const.TXT_FICHA}" aria-controls="{$smarty.const.TXT_FICHA}"
                   role="tab" data-toggle="tab"><i class="fa fa-folder fa-fw"></i> {$smarty.const.TXT_FICHA}</a>
            </li>
            <li role="presentation">
                <a href="#indics_correl" title="{$smarty.const.FIELD_INDICS} ({$smarty.const.FIELD_PROC})"
                   aria-controls="{$smarty.const.FIELD_INDICS} ({$smarty.const.FIELD_PROC})" role="tab"
                   data-toggle="tab"><i class="fa fa-dashboard fa-fw"></i><sub
                            class="fa fa-gear fa-fw"></sub> {$smarty.const.FIELD_INDICS} ({$smarty.const.FIELD_PROC}
                    )</a>
            </li>
            <li role="presentation">
                <a href="#indics_control" title="{$smarty.const.FIELD_INDICS} ({$smarty.const.TXT_CONTROL})"
                   aria-controls="{$smarty.const.FIELD_INDICS} ({$smarty.const.TXT_CONTROL})" role="tab"
                   data-toggle="tab"><i class="fa fa-dashboard fa-fw"></i><sub
                            class="fa fa-sliders fa-fw"></sub> {$smarty.const.FIELD_INDICS} ({$smarty.const.TXT_CONTROL}
                    )</a>
            </li>
            <li role="presentation">
                <a href="#objop_res" title="{$smarty.const.TXT_SEGUIMIENTO}"
                    ...
                    <div class="col-md-12">
                        <table class="table table-striped table-hover ficha">
                            ...
                            <tr>
                                ...
                                <td><span class="label label-default">{$objop->objest->linea->indice}
                                        .{$objop->objest->indice}.{$objop->indice}</span></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a title="{$smarty.const.FIELD_OBJ_EST} {$linea->indice}.{$objest->indice}. {$objest->nombre}"
                                       href='index.php?page=objest_mostrar&id_objest={$objest->id}&id_entidad={$entidad->id}'>{$linea->indice}
                                        .{$objest->indice}. {$objest->nombre}</a>
                                </td>
                            </tr>
                            <tr>
                                    ...
                                    {if $objetivo_subunidades}
                                        <ul class="list-unstyled">
                                            {foreach $objetivo_subunidades as $objetivo_subunidad}
                                                <li>
                                                    <a title="{$objetivo_subunidad->entidad->etiqueta}"
                                                       href="index.php?page=entidad_mostrar&id_entidad={$objetivo_subunidad->entidad->id}">{$objetivo_subunidad->entidad->etiqueta}</a>
                                                </li>
                                            {/foreach}
                                        </ul>
                                    {else}
                        ...
                        {if $_control || $responsable}
                            <div id="botones_ficha" class="btn-toolbar hidden" role="toolbar" aria-label="">
                                <a title="{$smarty.const.TXT_OBJOP_EDIT}" class="btn btn-default btn-danger"
                                ...
                                <a title="{$smarty.const.TXT_OBJOP_BORRAR}" class="btn btn-default btn-danger"
                                   href='javascript:void(0)' data-toggle="modal"
                                   data-target="#dialogo_confirmar_borrado_objop">
                        ...
                        <table id="tabla_indics_correl" class="table datatable table-striped table-hover">
                            ...
                            {foreach from=$indicadores_correlacion item=indicador}
                                <tr>
                                    ...
                                    <td>
                                        <a target="_blank" title="{$smarty.const.TXT_PROC_VER}"
                                           href='index.php?page=proceso_mostrar&id_proceso={$indicador->id_proceso}&id_entidad={$indicador->id_entidad}'>
                                            {$indicador->proceso->nombre}
                                        </a>
                                    </td>
                                    <td>{$indicador->periodicidad}</td>
                                    <td style="font-size: 12px">
                                        <a target="_blank" title="{$smarty.const.TXT_USER_PERFIL}"
                                    ...
                                    <td>
                                        <a target="_blank" class="btn btn-default btn-circle btn-xs"
                    ...
                    <li class="list-group-item list-group-item-info">
                        <div class="row">
                            <div class="col-sm-8 h4">
                                {$objop->objest->linea->indice}.{$objop->objest->indice}.{$objop->indice}
                                . {$objop->nombre}
                            </div>
                            <!-- /.col-sm-8 -->
                            <div class="col-sm-4">
